...
import { addManufacturer,updateManufacturer ,getManufacturer, cleanManfacturer} from "../../../Redux/features/ManufacturerReducer";
...
export default function AddManufacturer({show,setShow}) {   
  ...
  const manufacturerStatus = useSelector(
    (state) => state.manufacturer.status
  );
    ...
    async (data) => {
      if (!selectManufacturer) {
        ...
        if (!changedField.length) return toast.info("change some field first");
          ...
          dispatch(updateManufacturer({ id:selectManufacturer._id, newData: obj }));
        }
      }
    },
    [isDirty, dirtyFields]
  ...
  return (
    ...
    <Modal.Body>
         <form className="needs-validation"   onSubmit={handleSubmit((formData) => onSubmit(formData))}>
          <div className="row">
