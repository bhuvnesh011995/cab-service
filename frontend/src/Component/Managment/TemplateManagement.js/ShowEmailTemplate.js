...
const EmailTemplate = ({ template, show, setIsOpen }) => {
  return (
    <Modal show={show} onHide={() => setIsOpen(false)} size='lg'>
      ...
      <Modal.Body>
        <div>
          <div className='row'>
            ...
            <span className='col ms-2'>{template.title}</span>
          </div>
          <div className='row'>
            <h5 className='col-md-1'>Subject</h5>
            <span className='col ms-3'>{template.subject}</span>
          </div>
          <h5>Template Body</h5>
