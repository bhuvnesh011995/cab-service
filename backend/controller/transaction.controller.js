...
exports.getWalletBalance = async function (req,res,next){
    ...
    if(userType === "rider"){
        var user = await db.rider.findOne({_id:userId}).select({_id:0}).populate({path:"wallet",select:"balance"})
    }else if(userType ==="driver"){
        user = await db.driver.findOne({_id:userId}).select({_id:0}).populate({path:"wallet",select:{balcance:1}})
    }else {
        res.status(400).json({
            success:false,
...
exports.updateBalance = async function (req,res,next){
    ...
    try{
        await db.transaction.create({
            amount,type,description,wallet:user.wallet._id
        })
    }catch(error){
