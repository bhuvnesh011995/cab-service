...
exports.getAllManufacturer = async function (req, res,next){
  try{
    const manufacturer = await db.manufacturer.find({}).populate({ path: "model", select: "name" });
...
exports.updatemanufacturer = async function (req, res, next) {
  try {
      ...
      let manufacturer = await db.manufacturer.findOneAndUpdate({ _id:id}, { $set: obj},{new:true});    
